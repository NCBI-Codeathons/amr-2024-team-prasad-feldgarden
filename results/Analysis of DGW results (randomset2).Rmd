...
SELECT 
  asm_acc,
  COUNT(DISTINCT(dgw_combined.mutation_id)) OVER (PARTITION BY asm_acc) AS dgw_mutations_count,
  STRING_AGG(DISTINCT dgw_combined.element_symbol||dgw_combined.lesion) OVER (PARTITION BY asm_acc ORDER BY dgw_combined.lesion) AS dgw_lesions,
  STRING_AGG(DISTINCT microbigge.element_symbol) OVER (PARTITION BY asm_acc ORDER BY microbigge.element_symbol) AS mb_element_symbol,
FROM microbigge
